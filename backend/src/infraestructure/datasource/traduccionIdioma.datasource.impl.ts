...
export class TraduccionIdiomaDatasourceImpl implements TraduccionIdiomaDatasource {
  ...
  async getAll(): Promise<TraduccionIdiomaEntity[]> {
    const traduccionidiomas = await prisma.traduccionIdiomaModel.findMany();
    return traduccionidiomas.map( traduccionidioma => TraduccionIdiomaEntity.fromObject(traduccionidioma) );
  }
